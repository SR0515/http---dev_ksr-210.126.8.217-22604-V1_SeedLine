<script lang="ts">
    ...
    import WriteLayout from '$lib/components/store/write/WriteLayout.svelte'; //페이지 레이아웃
    import BasicInfoForm from '$lib/components/store/edit/BasicInfoForm.svelte';
    import PaymentForm from '$lib/components/store/edit/PaymentForm.svelte';
    import ServiceSettingForm from '$lib/components/store/edit/ServiceSettingForm.svelte';
    ...
    import { validatePassword,validateForm } from '$lib/utils/writeUtils'; //수정필요
    import { editUser } from '$lib/utils/editUtils' 
    import { fetchUserInfo, getDefaultStoreListData, passEditCheck } from '$lib/utils/editUtils'
    import { loadBankList, bankListData } from '$lib/types/bank';
    import { authStore } from '$lib/stores/auth';
    import { showToast } from '$lib/stores/toast';
    import type { StoreListData } from '$lib/types/store';
    import {confirmedCatTId, confirmedCatId, confirmedPayKey} from "$lib/utils/Duplicate_check";
    ...
    //검색할 가맹점 mid
    $: mid = $page.url.searchParams.get('mid') ?? '';
    ...
    // 상태 관리
    let loading = false;
    let userData: StoreListData = getDefaultStoreListData();
    ...
    async function loadUserData() {
        try {
            loading = true;
            userData = await fetchUserInfo(userId, mid, 'store');   
            originData = {
                catId: userData.store_wallet.ki_catId,
                payKey: userData.store_wallet.ki_paykey,
                catTId: userData.store_wallet.T_catId
            }
            if(userData.store_wallet?.T_catId_date){
                catIdDate = userData.store_wallet.T_catId_date.split(' ')[0].trim();
            }
            addressValue = userData.store_basic.adress
            detailValue = userData.store_basic.adress_detail
    ...
    try {
        ...
        const json = await response.json();
            if (json.success) {
                ...
                userData.settlement_commission = json.data.settlement_commission || 0;
            } else {
    ...
    function changeCatId(init: string) {
        let inputElement = document.getElementById("kitidCheckBtn");
        if (!inputElement) return;

        if(originData.catId === init || confirmedCatId === init) {
            inputElement.innerText = "확인완료"
            inputElement.style.display = "none";
        }else {
            inputElement.innerText = "중복확인"
            inputElement.style.display = "block";
        }
    }

    function changePaykey(init: string) {
        let inputElement = document.getElementById("kipaykeyCheckBtn");
        if (!inputElement) return;
        
        if(originData.payKey === init || confirmedPayKey === init ) {
            inputElement.innerText = "확인완료"
            inputElement.style.display = "none";
        }else{
            inputElement.innerText = "중복확인"
    ...
    function changeTCatId(init: string) {
        let inputElement = document.getElementById("tidCheckBtn");
        if (!inputElement) return;
        if(originData.catTId === init || confirmedCatTId === init) {
            inputElement.innerText = "확인완료"
            inputElement.style.setProperty("display", "none", "important");
        }else{
            inputElement.innerText = "중복확인"
            inputElement.style.display = "block";
        }
    ...
    async function handleSubmit() {
        ...
        try {
            ...
            const success = await editUser('store', editData);
            
            if (success) {                
                showToast('가맹점 수정이 완료 되었습니다.', 'success');
                disabledValue = true;
                setTimeout(() => {
                    goto('/list/store');
                }, 1500);
    ...
    onMount(async () => {
        ...
        let matched = bankListData.find(
            bank => bank.bank_name === userData.store_wallet.bank_name
        );
...
<WriteLayout pageType={pageType}>
     ...
     <form id="writeForm" method="post" on:submit|preventDefault={handleSubmit}>
        ...
        <!-- 결제수단 설정 -->
         <PaymentForm
            bind:userData={userData}
            bind:settlement_commission={userData.settlement_commission}
            bind:catIdDate={catIdDate}
            disabled={loading}
            {originData}
            onChangeCatId={changeCatId}
            onChangePaykey={changePaykey}
            onChangeTCatId={changeTCatId}
        />    
