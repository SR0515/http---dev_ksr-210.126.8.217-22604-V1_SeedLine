<script lang="ts">
    ...
    import { EDIT_SEARCH_OPTIONS } from '$lib/constants/searchOptions';
    import { useLogData } from '$lib/hooks/useLogData';
    import type { EditLogData } from '$lib/types/logs';
    
    // 커스텀 훅 사용
    const logDataStore = useLogData('EditLog', 'id');
...
<LogsSearch 
    ...
    searchOptions={EDIT_SEARCH_OPTIONS}
    showTypeSelect={true}
/>
    ...
    <div id="table_wrap" class="table_wrap">
        <table class="table_list" width="100%">
            ...
            <tbody>
                ...
                {:else}
                    {#each logLists as log, index (`${log.id}_${log.edit_date}_${index}`)}
                        <tr>
                            <td>{pagination.totalCount - ((pagination.currentPage - 1) * pagination.pageSize) - index}</td>
                            <td>{formatters.getTypeLabel(log.type)}</td>
                            ...
                            <td style="overflow: hidden; max-width: 150px;">{log.url || ''}</td>
                            <td>{log.edit_date || ''}</td>
                        </tr>
                    {/each}
                {/if}
