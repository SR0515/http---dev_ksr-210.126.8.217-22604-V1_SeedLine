<script lang="ts">
    ...
    import { SETTLEMENT_SEARCH_OPTIONS } from '$lib/constants/searchOptions';
    import type { SettlementLogData } from '$lib/types/logs';
    
    // 상태 관리
    let loading = false;
    let error: string | null = null;
    let logLists: SettlementLogData[] = [];
    let currentPage = 1;
    let totalPages = 1;
    let totalCount = 0;
    let pageSize = 10;
    ...
    async function loadLogData() {
        ...
        try {
            ...
            if (mounted) {
                ...
                error = `가맹점 정산처리 로그 데이터를 불러오는데 실패했습니다: ${errorMessage}`;
...
<LogsSearch 
    ...
    searchOptions={SETTLEMENT_SEARCH_OPTIONS}
                ...
                {:else}
                    {#each logLists as log, index}
                        <tr>
                            ...
                            <td>
                                {#if log.pre_value}
                                    {@const preStatus = formatters.getStatusLabel(log.pre_value)}
                                    <span class={preStatus.className}>{preStatus.text}</span>
                                {/if}
                            </td>
                            <td>
                                {#if log.to_value}
                                    {@const toStatus = formatters.getStatusLabel(log.to_value)}
