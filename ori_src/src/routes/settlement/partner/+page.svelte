<script lang="ts">
	...
	// 파트너 정산 데이터 인터페이스
	interface PartnerSettlementData {
		id: number;
	...
	// 데이터 상태
	let settlementList: PartnerSettlementData[] = [];
	let loading = false;
	...
	onMount(async () => {
		...
		const hasAccess = await checkAuth('settlement_partner');
		if (!hasAccess) {
		...
		endDate = `${today.getFullYear()}-${(today.getMonth() + 1).toString().padStart(2, '0')}-${new Date(today.getFullYear(), today.getMonth() + 1, 0).getDate()}`;
	...
	async function fetchData() {
		...
		try {
			...
			const response = await fetch(`/api/settlement/partner_list?${params}`);
	...
	function navigateToDetail(settlementId: number) {
		goto(`/settlement/partner/${settlementId}`);
	}
	...
	// 정산 제목 생성
	function generateSettlementTitle(settlement: PartnerSettlementData): string {
		const [year, month] = settlement.settlement_month.split('-');
		return `${year}년 ${month}월 파트너 정산내역`;
	}
	...
	async function downloadExcel() {
		try {
			...
			const response = await fetch(`/api/settlement/partner_excel?${params}`);
			...
			if (response.ok) {
				...
				a.href = url;
				a.download = `파트너정산목록_${startDate}_${endDate}.xlsx`;
					...
					<div id="table_wrap" class="table_wrap">
						<table class="table_list settlement_table" width="100%">
							...
							<colgroup>
								...
								<col style="width: 35%;"> <!-- 정산 내역 -->
								<col style="width: 10%;"> <!-- 파트너등급 -->
								<col style="width: 10%;"> <!-- 가맹점수 -->
								<col style="width: 12%;"> <!-- 총 수수료 -->
									...
									{#each settlementList as settlement, index}
										<tr>
											...
											<td class="settle_title">
												...
												{#if settlement.partner_name}
													<div class="partner-info">
														{settlement.partner_name}
													</div>
												{/if}
											</td>
											<td class="text-center partner-rate">
												{settlement.partner_rate}
											</td>
											<td class="text-center">{settlement.store_count}개</td>
											<td class="text-right">{formatAmount(settlement.total_commission)}</td>
											<td class="text-right settlement-amount">
												{formatAmount(settlement.settlement_amount)}
											</td>
											<td class="text-center">{settlement.settlement_month}</td>
										</tr>
									{/each}
								{/if}
							</tbody>
