...
let partnerLevels: PartnerLevel[] = [];
let partners: any[] = [];
let permissions: Record<number, Permission> = {};
let specialPaths: Record<number, SpecialPath> = {};
...
function setPartnerLevels(data: any[]) {
...
async function fetchLeftMenuData(): Promise<void> {
    try {
...
declare global {
    interface Window {
        openPermissionModal: (levelId: number) => void;
        closePermissionModal: () => void;
        savePermissions: () => void;
        updatePathDisplay: () => void;
        editLevelName: (levelId: number) => void;
        saveLevelName: (levelId: number) => void;
        deleteLevel: (levelId: number) => void;
        showConfirm: (message: string, onConfirm: () => void) => void;
        closeConfirmModal: () => void;
        handleKeyPress: (event: KeyboardEvent, levelId: number) => void;
    }
}
...
function createLevelCard(level: PartnerLevel, index: number): HTMLDivElement {
    const div = document.createElement('div');
    div.className = 'level-card';
    div.dataset.level = level.order.toString();
    div.dataset.levelId = level.id.toString();
...
function editLevelName(levelId: number) {
...
function handleKeyPress(event: KeyboardEvent, levelId: number): void {
    if (event?.key === 'Enter') {
...
async function saveLevelName(levelId: number): Promise<void> {
    const input = document.getElementById(`edit-${levelId}`) as HTMLInputElement | null;
    if (!input) return;
...
function cancelEdit(levelId: number): void {
    const level = partnerLevels.find(l => l.id === levelId);
    const header = document.getElementById(`header-${levelId}`);
    
    if (!header || !level) return;
    const card = header.closest('.level-card');
    if (card) card.classList.remove('editing');
...
function showConfirm(message: string, onConfirm: () => void): void {
    ...
    const confirmBtn = document.getElementById('confirmButton') as HTMLElement;
