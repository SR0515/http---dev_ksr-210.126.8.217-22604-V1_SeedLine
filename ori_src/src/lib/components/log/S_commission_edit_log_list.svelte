<script lang="ts">
    ...
    type logItem = {
        ...
        store_name: string;
        store_catid: string;
        pre_prime_AC: number | null;
        to_prime_AC: number | null;
        pre_prime_BC: number | null;
        to_prime_BC: number | null;
        pre_prime_CC: number | null;
        to_prime_CC: number | null;
        pre_prime_C: number | null;
        to_prime_C: number | null;
        editor_name: string;
        editor_ip: string;
        editor_browser: string;
        url: string;
        edit_date: string; // ISO 8601 형식의 날짜 문자열
    }
    ...
    async function fetchData() {
        ...
        try {  
            ...
            const response = await fetch(`/api/log/StoreCommissionLog?` + params.toString());
...
<div id="content" class="container">
    ...
    <section class="borderbox form_view">
        <div class="title_wrap">
            <h5 class="page_title">로그 - 가맹점 수수료 수정</h5>
        </div>
        <TabMenu/>
        <form  id="searchForm">
        ...
        <div id="table_wrap" class="table_wrap">
            <table class="table_list" width="100%">
                ...
                <colgroup>                     
                    ...
                    <col style="width: 6.67% !important;"> <!-- 가맹점 아이디 -->
                    <col style="width: 6.67% !important;"> <!-- 가맹점 명 -->
                    <col style="width: 6.67% !important;"> <!-- TID -->
                    <col style="width: 8.67% !important;"> <!-- 본사 수수료 변동 -->
                    <col style="width: 8.67% !important;"> <!-- 대행사 수수료 변동 -->
                    <col style="width: 9.67% !important;"> <!-- 에이전시 수수료 변동 -->
                    <col style="width: 9.67% !important;"> <!-- 가맹점 수수료 변동 -->
                    <col style="width: 6.67% !important;"> <!-- 수정 관리자명 -->
                    <col style="width: 6.67% !important;"> <!-- 수정 IP -->   
                    <col style="width: 6.67% !important;"> <!-- 수정 브라우저 -->
                    <col style="width: 6.67% !important;"> <!-- 수정 URL -->
                ...
                <thead>
                    <tr>
                        ...
                        <th rowspan="1" scope="col" >가맹점아이디</th>
                        <th rowspan="1" scope="col" >가맹점명</th>
                        <th rowspan="1" scope="col" >TID</th>
                        <th rowspan="1" scope="col" >본사 수수료 변동(%)</th>
                        <th rowspan="1" scope="col" >대행사 수수표 변동(%)</th>
                        <th rowspan="1" scope="col" >에이전시 수수료 변동(%)</th>
                        <th rowspan="1" scope="col" >가맹점 수수료 변동(%)</th>
                        <th rowspan="1" scope="col" >수정 관리자명</th>
                ...
                <tbody>             
                    {#if loading}
                    <tr>
                        <td colspan="13" class="loading">데이터를 불러오는 중...</td>
                    </tr>    
                    ...
                    <tr>
                        ...
                        <td class="wd_date"> {dist.store_id}</td>                                       
                        <td> {dist.store_name} </td>                                   
                        <td> {dist.store_catid} </td>                                                        
                        <td class="wd_pre_prime">                                                            
                            <p>{dist.pre_prime_AC}<span style="color: #999;">&#8594</span> <span style="color: red;">{dist.to_prime_AC}</span> </p>
                        </td>
                        <td class="wd_pre_prime">                                                            
                            <p>{dist.pre_prime_BC || ''}<span style="color: #999;">&#8594</span> <span style="color: red;">{dist.to_prime_BC || ''}</span> </p>
                        </td>
                        <td class="wd_pre_prime">                                                            
                            <p>{dist.pre_prime_CC || ''}<span style="color: #999;">&#8594</span> <span style="color: red;">{dist.to_prime_CC || ''}</span> </p>
                        </td>
                        <td class="wd_pre_prime">                                                            
                            <p>{dist.pre_prime_C || ''}<span style="color: #999;">&#8594</span> <span style="color: red;">{dist.to_prime_C || ''}</span> </p>
                        </td>
                        <td> {dist.editor_name}</td>                                                                                                    
