...
import type { SettlementSearchData, StoreSettlementData } from '$lib/types/settlement';
import { fail, redirect } from '@sveltejs/kit';

// 더미 데이터 (실제로는 데이터베이스에서 가져옴)
const getDummySettlements = (): StoreSettlementData[] => [
    {
        id: '1',
...
export const load = async ({ url }: Parameters<PageServerLoad>[0]) => {
    ...
    let settlements = getDummySettlements();
...
export const actions = {
    search: async ({ request, url }: import('./$types').RequestEvent) => {
    ...
    reset: async ({ url }: import('./$types').RequestEvent) => {
